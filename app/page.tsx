...
import { findCurrentStepIndex } from "../utils/productionUtils";
import { FiLayers, FiAlertCircle, FiCheckCircle, FiClock } from "react-icons/fi";
...
export default function WorkerInputForm({
  ...
  const {
    ...
    allSteps,
    currentStepIndex,
  } = useLayerProcessing({ stepId, stepName });
  ...
  // Use the hooks to get layers and step information
  const { layers, isLoading: isLayersDataLoading, error: layersError } = useProductLayers();
  const { steps, isLoading: isStepsLoading, error: stepsError } = useSteps();
  
  // State for filtered layers that match the current step
  const [currentStepLayers, setCurrentStepLayers] = useState<any[]>([]);

  // Filter layers based on current step
  useEffect(() => {
    if (!stepId || isLayersDataLoading || !layers) return;
    
    // Filter layers where currentStep matches the worker's stepId
  ...
  }, [stepId, layers, isLayersDataLoading]);
  ...
  const displayAllSteps = useFakeData ? fakeAllSteps : allSteps;
  const displayCurrentStepIndex = useFakeData ? fakeCurrentStepIndex : currentStepIndex;
  ...
  const renderProductionInfo = () => {
    ...
    return (
      <div className="bg-gray-50 rounded-lg p-6 mb-6 border border-gray-200">
        ...
        {/* Steps Progress */}
        {displayAllSteps && displayAllSteps.length > 0 && (
          <div>
            ...
            <div className="relative">
              ...
              <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${((displayCurrentStepIndex + 1) / displayAllSteps.length) * 100}%` }}
                ></div>
              </div>
              
              {/* Steps */}
              <div className="flex justify-between mt-2">
                {displayAllSteps.map((step: { _id: string; name: string }, index: number) => (
                  <div 
                    key={step._id} 
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden max-w-4xl mx-auto">
      ...
      <div className="p-6">
        ...
        {/* Layers in this step - New section showing all layers in the current step */}
        <div className="mb-6">
          ...
          {isLayersDataLoading ? (
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
              <div className="flex items-center">
                ...
                <p>Error loading layers: {layersError}</p>
              </div>
            </div>
          ) : currentStepLayers.length > 0 ? (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentStepLayers.map((layer) => {
                    ...
                    // Get arrival time
                    const arrivalTime = layer.currentStep && typeof layer.currentStep === 'object' && layer.currentStep.startTime
        ...
        {/* Production Line Layers Summary */}
        {productionLine && (
          <div className="mt-6">
            <h3 className="text-lg font-medium text-gray-900 mb-3 flex items-center">
              ...
              {displayAllSteps && displayAllSteps.length > 0 ? (
                <div className="p-4">
                  <div className="flex flex-wrap gap-2">
                    {displayAllSteps.map((step: any, index: number) => {
                      // Count layers in this step
                      const layersInStep = stepsLayers.filter(
                        (layer: any) => 
                          layer.currentStep && 
                          (typeof layer.currentStep === 'object' 
                            ? layer.currentStep.stepId === step._id
                            : layer.currentStep === step._id)
                      ).length;
                      
                      const isCurrentStep = index === displayCurrentStepIndex;
                ...
                <p className="p-4 text-center text-gray-500">
                  No steps defined for this production line
                </p>
              )}
            </div>
          </div>
