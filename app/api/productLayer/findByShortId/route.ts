...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!matchingLayer) {
      ...
      console.log("Available IDs:", allLayers.map(layer => layer._id.toString().substring(0, 10)));
      return NextResponse.json({ error: "No matching layer found" }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: "Failed to find layer" }, { status: 500 });
  }
}
