...
export async function DELETE(request: NextRequest) {
  const id = request.headers.get("id") || '';
  if (!id) {
    return NextResponse.json({ error: "Product layer ID is required" }, { status: 400 });
  }
  await connect();
  try {
    ...
    return NextResponse.json({ message: "Product layer deleted successfully" }, { status: 200 });
  } catch (error) {
    console.error("Error deleting product layer:", error);
    return NextResponse.json({ error: "Failed to delete product layer" }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const result = await ProductLayer.findByIdAndUpdate(id, body, { new: true, runValidators: true });
    if (!result) {
      return NextResponse.json({ error: "Product layer not found" }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: "Failed to update product layer" }, { status: 500 });
  }
}