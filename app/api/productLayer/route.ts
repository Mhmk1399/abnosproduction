...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.productionCode) {
      return NextResponse.json({ error: "Production code is required" }, { status: 400 });
    }

    if (!body.width || typeof body.width !== 'number' || body.width <= 0) {
      return NextResponse.json({ error: "Valid width is required" }, { status: 400 });
    }
    ...
    if (body.currentStep && !isValidObjectId(body.currentStep)) {
      return NextResponse.json({ error: "Invalid current step ID" }, { status: 400 });
    }

    if (body.currentInventory && !isValidObjectId(body.currentInventory)) {
      return NextResponse.json({ error: "Invalid current inventory ID" }, { status: 400 });
    }

    if (body.designNumber && !isValidObjectId(body.designNumber)) {
      return NextResponse.json({ error: "Invalid design number ID" }, { status: 400 });
    }
    ...
    if (body.treatments && Array.isArray(body.treatments)) {
      for (const treatment of body.treatments) {
        if (!treatment.treatment || !isValidObjectId(treatment.treatment)) {
          return NextResponse.json({ error: "Invalid treatment ID in treatments array" }, { status: 400 });
        }

        if (treatment.count && (typeof treatment.count !== 'number' || treatment.count < 0)) {
          return NextResponse.json({ error: "Invalid count in treatments array" }, { status: 400 });
        }
      }
    }
    ...
    if (body.glass) {
      ...
      if (!glassExists) {
        return NextResponse.json({ error: "Referenced glass not found" }, { status: 404 });
      }
    }
    ...
    if (body.productionLine) {
      ...
      if (!lineExists) {
        return NextResponse.json({ error: "Referenced production line not found" }, { status: 404 });
      }
    }
    ...
    if (body.currentStep) {
      ...
      if (!stepExists) {
        return NextResponse.json({ error: "Referenced step not found" }, { status: 404 });
      }
    }
    ...
    return NextResponse.json({ error: "Failed to create product layer" }, { status: 500 });
  }
}
