...
export async function PATCH(request: NextRequest) {
    ...
    try {
        ...
        const result = await StepExecution.findByIdAndUpdate(id, body, { new: true, runValidators: true });
        if (!result) {
        ...
        return NextResponse.json({ error: "Failed to update step execution" }, { status: 500 });
    }
}
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ message: "Step execution deleted successfully" }, { status: 200 });
    } catch (error) {
        console.error("Error deleting step execution:", error);
        return NextResponse.json({ error: "Failed to delete step execution" }, { status: 500 });
    }
}