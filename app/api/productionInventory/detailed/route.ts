...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ message: "Inventory deleted successfully" }, { status: 200 });
...
export async function PATCH(request: NextRequest) {
    ...
    try {
        ...
        const result = await productionInventory.findByIdAndUpdate(id, body, { new: true, runValidators: true });
        if (!result) {
            return NextResponse.json({ error: "Inventory not found" }, { status: 404 });
        }
