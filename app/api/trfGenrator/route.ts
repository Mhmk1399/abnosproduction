...
function generateTrfContent(layers: layerData[], batchNumber: number): string {
  ...
  console.log(layersByOrder,"layer by order")

  // Process each order group
  Object.entries(layersByOrder).forEach(([orderKey, orderLayers]) => {
    console.log(orderKey);
    const firstLayer = orderLayers[0];
    //here we must get an english name from the customer and asigne that 
    const customerName = firstLayer.customer?.name || "Unknown";
    //here we must extract the code using the designe number mongoose id we must create this object in the data base
    const orderNumber = firstLayer.designNumber|| `ORD${batchNumber}`;
    //here again we must get the customer code not the customer id 
    ...
    content += `<ORD> ${padRight(
      orderNumber.toString().substring(0, 10),
      10
    )} ${padRight(customerNumber.toString().substring(0, 10), 10)} ${padRight(
      customerName.substring(0, 40),
      40
    )} ${productionDate} ${currentDate} Gr\r\n`;
    ...
    orderLayers.forEach((layer, index) => {
      ...
      // Add POS record
      content += `<POS> ${padRight(itemNum.toString(), 6)} ${padRight(
        idNum,
        10
      )} 0000 ${padRight(qty.toString(), 1)} ${padRight(
        width.toString(),
        6
      )} ${padRight(height.toString(), 6)} ${padRight(
        glassType,
        10
      )} ${padRight("", 25)}000 00 00 00 0 0 00000 0\r\n`;

      // Add TXT record with additional information
      const txtInfo = `${itemNum}-${layer.productionCode || ""}`;
      const companyName = "ABNOOS JAM KARAJ";

      content += `<TXT> ${padRight(txtInfo, 80)} ${padRight(
        companyName,
        80
      )} ${padRight("", 80)} ${padRight(layer.productionCode || "", 80)}\r\n`;

      // Add shape information if available
      if (layer.designNumber) {
        content += `<SHP> ${padRight(itemNum.toString(), 6)} ${padRight(
          width.toString(),
          6
        )} ${padRight(height.toString(), 6)} 0\r\n`;
      }
