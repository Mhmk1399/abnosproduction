...
export const recordProcessEvent = async (request: NextRequest, layerId: string) => {
    try {
        await connect();
        ...
        if (body.eventType === 'start') {
            ...
            const historyEntry = layer.processHistory
                .filter((h: { stepId: { toString: () => any; }; status: string; }) => 
                    h.stepId.toString() === body.stepId.toString() && h.status === 'in-progress')
                .pop();
