...
export class ProductionTrackerService {
     ...
     */
    static async recordStepExecution(layerId: string, stepId: string, productionLineId: string, passed: boolean = true, notes?: string) {
        try {
            ...
            const stepExecution = {
                ...
                ...(layer.treatments && layer.treatments.length > 0 && {
                    treatmentsApplied: layer.treatments.map((t: { treatment: glasstreatments; count: number; measurement: string }) => ({
                        treatment: typeof t.treatment === 'object' ? t.treatment._id : t.treatment,
                        count: t.count || 1,
    ...
    static async moveToNextStep(layerId: string) {
        try {
            ...
            const productionLine = typeof layer.productionLine === 'object'
                ? layer.productionLine
                : null;

            if (!productionLine || !productionLine.steps || productionLine.steps.length === 0) {
            ...
            const currentStepIndex = productionLine.steps.findIndex((s: { step: { _id: string; }; }) => {
            ...
            const nextStepId = typeof nextStep.step === 'object' ? nextStep.step._id : nextStep.step;
    ...
    static async getProductionQueue(productionLineId: string) {
        try {
            ...
            const queueLayers = layers.filter((layer: layerData) => {
                const lineId = typeof layer.productionLine === 'object'
            ...
            const queue = queueLayers.reduce((acc: { [key: string]: layerData[] }, layer: layerData) => {
