...
export function buildStepsList(productionLine: any): any[] {
  ...
  if (!productionLine.microLines || !Array.isArray(productionLine.microLines)) {
    console.log(
      "No micro lines found in production line or microLines is not an array"
    );
    console.log(
      "Production line data:",
      JSON.stringify(productionLine, null, 2)
    );
    return [];
  }

  // Sort microLines by order if available
  const sortedMicroLines = [...productionLine.microLines].sort(
    (a, b) => (a.order || 0) - (b.order || 0)
  );

  console.log(
    `Found ${sortedMicroLines.length} micro lines in production line`
  );
  ...
  sortedMicroLines.forEach((microLineRef: any, microLineIndex: number) => {
    console.log(
      `Processing micro line ${microLineIndex + 1}/${sortedMicroLines.length}`
    );
    ...
    if (!microLine.steps || !Array.isArray(microLine.steps)) {
      console.log(
        `Micro line ${microLineIndex + 1} has no steps or steps is not an array`
      );
      return;
    }

    // Sort steps by order if available
    const sortedSteps = [...microLine.steps].sort(
      (a, b) => (a.order || 0) - (b.order || 0)
    );
    ...
    sortedSteps.forEach((stepRef: any, stepIndex: number) => {
      if (!stepRef) {
        console.log(
          `Step reference ${stepIndex + 1} in micro line ${
            microLineIndex + 1
      ...
      if (!step) {
        console.log(
          `Step ${stepIndex + 1} in micro line ${
            microLineIndex + 1
          } is undefined or null`
        );
        return;
      ...
      if (typeof step === "string") {
        ...
        console.log(
          `Adding step: ${step.name || "unnamed"} (${stepIndex + 1}/${
            sortedSteps.length
          })`
        );
        allSteps.push({
  ...
  if (allSteps.length > 0) {
    console.log(
      "Steps list:",
      allSteps.map((step) => step._id || "unknown").join(", ")
    );
  }
...
export function needsOptimization(layer: any): boolean {
  ...
  if (!layer.currentStep && layer.productionLine) {
    console.log(
      "Layer has no current step, checking first step of production line"
    );
...
export function logStepPath(layer: any): string {
  ...
  const currentStepIndex = layer.currentStep
    ? findCurrentStepIndex(stepsList, layer.currentStep._id)
    : -1;

  const path = stepsList
    .map((step, index) => {
