...
const DynamicTable: React.FC<DynamicTableProps> = ({
  ...
  const filteredData = useMemo(() => {
    // If onFilterChange is provided, filtering is handled server-side
    if (onFilterChange) {
      return sortedData;
    }
  ...
  const clearAllFilters = () => {
    ...
    // Ensure the parent component gets notified with empty filters
    if (onFilterChange) {
  ...
  const renderFilterField = (field: FilterField) => {
    ...
    switch (field.type) {
      ...
      case "numberRange":
        const numRange = Array.isArray(value) ? value as [number, number] : [null, null] as [number | null, number | null];
        return (
          <div className="flex gap-2">
            <input
              type="number"
              value={numRange[0] !== null ? String(numRange[0]) : ""}
              onChange={(e) =>
            ...
            <input
              type="number"
              value={numRange[1] !== null ? String(numRange[1]) : ""}
              onChange={(e) =>
                updateFilter(field.key, [
                  Number(numRange[0]) || 0,
                  Number(e.target.value) || 0,
      ...
      case "dateRange":
        const dateRange = Array.isArray(value) ? value as [string, string] : ['', ''] as [string, string];
        return (
          <div className="flex gap-2">
